...
import { useAddNewCategoryMutation, useGetParentCategoryListQuery, useUpdateCategoryMutation } from "../../../store/masters/mastersApis";
import StyledTextarea from "../../Common/StyledTextarea";
import BasicSelect from "../../Common/Select";

export const CategoryAddDrawer = ({ initialValues, show, close, formType }) => {
    const dispatch = useDispatch();
    const [addNewCategory, { isLoading: addCategoryLoading }] = useAddNewCategoryMutation();
    const [updateCategory, { isLoading: updateCategoryLoading }] = useUpdateCategoryMutation();

    const { data: parentCategory = [], isLoading: parentCategoryLoading } = useGetParentCategoryListQuery({}, { skip: !show });
                ...
                >
                    <Grid container sx={{ display: "flex" }} direction={"column"} width={"100%"} height={"100%"} >
                        <Grid container flex={0} px={1} py={1} borderBottom={1} borderColor={"rgba(5, 5, 5, 0.06)"}>
                            ...
                            <Grid item alignSelf={"center"}>
                                <Typography variant="h6">{formType} Category</Typography>
                            </Grid>
                        </Grid>
                                ...
                                <Grid item xs={12} md={6}>
                                    <BasicSelect
                                        ...
                                        items={[
                                            ...
                                            ...(formType === "Edit" ? parentCategory.filter(item => item.value !== values?._id) : parentCategory)
                                        ]}
                                        disabled={disabled}
                                    />
                                </Grid>
                                ...
                                <Grid flexShrink={0} borderTop={1} borderColor={"rgba(152, 188, 252, 0.16)"} sx={{ padding: "8px 16px" }}>
                                    <Grid sx={{ display: "flex", justifyContent: "flex-end" }}>
                                        <Grid sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                                            ...
                                            <Button size="large" type="submit" loading={addCategoryLoading || updateCategoryLoading} onClick={() => handleSubmit()}>Save</Button>
                                        </Grid>
                                    </Grid>
                                </Grid>
                                :
